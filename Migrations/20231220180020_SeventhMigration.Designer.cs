// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Salon_Infrumusetare.Data;

#nullable disable

namespace Salon_Infrumusetare.Migrations
{
    [DbContext(typeof(Salon_InfrumusetareContext))]
    [Migration("20231220180020_SeventhMigration")]
    partial class SeventhMigration
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.23")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("Salon_Infrumusetare.Models.Client", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<string>("Nume")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Prenume")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ServiciuSelectatID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("ServiciuSelectatID");

                    b.ToTable("Client");
                });

            modelBuilder.Entity("Salon_Infrumusetare.Models.Serviciu", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<int?>("ClientID")
                        .HasColumnType("int");

                    b.Property<int>("Minute")
                        .HasColumnType("int");

                    b.Property<string>("Nume")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("Pret")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int?>("SpecialistID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("ClientID");

                    b.HasIndex("SpecialistID");

                    b.ToTable("Serviciu");
                });

            modelBuilder.Entity("Salon_Infrumusetare.Models.Specialist", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"), 1L, 1);

                    b.Property<int>("AnVechime")
                        .HasColumnType("int");

                    b.Property<string>("Nume")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Prenume")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Specialitate")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.ToTable("Specialist");
                });

            modelBuilder.Entity("Salon_Infrumusetare.Models.Client", b =>
                {
                    b.HasOne("Salon_Infrumusetare.Models.Serviciu", "ServiciuSelectat")
                        .WithMany()
                        .HasForeignKey("ServiciuSelectatID");

                    b.Navigation("ServiciuSelectat");
                });

            modelBuilder.Entity("Salon_Infrumusetare.Models.Serviciu", b =>
                {
                    b.HasOne("Salon_Infrumusetare.Models.Client", null)
                        .WithMany("Servicii")
                        .HasForeignKey("ClientID");

                    b.HasOne("Salon_Infrumusetare.Models.Specialist", "Specialist")
                        .WithMany()
                        .HasForeignKey("SpecialistID");

                    b.Navigation("Specialist");
                });

            modelBuilder.Entity("Salon_Infrumusetare.Models.Client", b =>
                {
                    b.Navigation("Servicii");
                });
#pragma warning restore 612, 618
        }
    }
}
